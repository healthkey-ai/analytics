...
def test_compute_returns_funnel_structure():
    ...
    def fake_filters(request, qs=None, funnel=None):
        funnel.append({"key": "disease_stage", "label": "Disease & stage", "count": 250})
        return qs

    with patch("metrics.services.eligibility.apply_cohort_filters", side_effect=fake_filters) as acf:
        result = compute(_request(), base)

    assert result["total"] == 1000
    # No filters apply after the last recorded step, so eligible == last step's count
    assert result["eligible"] == 250
    assert result["eligible_pct"] == 25.0
    assert result["steps"] == [
        {"key": "all", "label": "All patients", "count": 1000},
        {"key": "disease_stage", "label": "Disease & stage", "count": 250},
    ]
...
def test_eligible_pct_zero_when_population_empty():
    ...
    with patch("metrics.services.eligibility.apply_cohort_filters", return_value=base):
        result = compute(_request(), base)
...
def test_metrics_includes_eligibility_in_payload(api_client):
    ...
    # The view shares its org-scoped base with the eligibility service so the
    # scope is evaluated once per request.
    service_mocks["eligibility"].compute.assert_called_once()
    assert service_mocks["eligibility"].compute.call_args.args[1] is fake_base
...
def test_metrics_includes_eligibility_when_cohort_empty(api_client):
    ...
    with patch("metrics.views.apply_org_scope", return_value=(fake_base, None)), \
         patch("metrics.views.apply_cohort_filters", return_value=fake_qs), \
    ...
    assert resp.data["eligibility"] == elig_value
    mock_elig.compute.assert_called_once()
    assert mock_elig.compute.call_args.args[1] is fake_base
